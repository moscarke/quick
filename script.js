...
function depPlace(line){
	...
	for (let i = 0; i < response.length; i++){
		if (response[i]["line"] == line){
			...
			x += "<button class='btnThree' style='width:32%' onclick='schQuery(\"" + response[i]["code"] + "\",\"" + response[i]["line"] + "\");'>" + response[i]["name"] + "</button>"
			sequence++;
			continue;
		}
		if (start){
